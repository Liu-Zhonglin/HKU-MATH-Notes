...
\begin{document}
	...
	By the Uniformization Theorem, any simply connected Riemann surface is
...
A holomorphic $1$‑form on \(X\) can be lifted to a holomorphic $1$‑form 
...
More generally, for an integer \(k \ge 1\), a holomorphic $k$‑differential
...
\begin{definition}[Holomorphic $k$‑differential on $X = \mathbb{D}/\Gamma$]
	...
	= f(\gamma z)\,(\gamma'(z))^k\,(dz)^k=f(z)(dz)^k.
	\]
\end{definition}
...
= \sum_{\gamma \in \Gamma} h(\gamma z)\,(\gamma'(z))^k
\]
is absolutely and uniformly convergent on compact subsets of~\(\mathbb{D}\).
