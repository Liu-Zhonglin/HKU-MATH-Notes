...
\end{remark}
    ...
    \begin{remark}
        The sequence $\mathbb{P_1}(X_n=0)$ is strictly increasing and bounded above,
        the convergence follows from the completeness axiom of real numbers.
    \end{remark}
...
\end{figure}
	...
	\begin{remark}
		Probability generating functions are defined for nonnegative integer-valued random variables only (\textcolor{red}{In this course, this is obviously true, but in general? I don't think so, because it is possible to define Laurent expansion. For Laurent expansion, the probability distribution is the residue at $z=0$. The expectation is the derivatives at $z=1$.}). They encode the full probability distribution of \(X\), since the coefficient of \(s^k\) is \(\mathbb{P}(X=k)\).
	\end{remark}
	...
	\begin{proposition}[Factorial moments]
		Let \(X\) be a nonnegative integer-valued random variable with p.g.f. \(G_X\). \textcolor{red}{assume that $z=1$ lies in the radius of convergence.} Then
		\[
		G_X'(1)=\mathbb{E}[X],
		\]
		and more generally,
	...
	\end{proof}
	\begin{remark}
	    \textcolor{red}{This is called the factorial moment. Using the Stirling numbers of the second kind, we may convert the factorial moment to the usual moment.}
	\end{remark}
	...
	\begin{proof}
		...
		\sum_{i=1}^{X_n}\xi_{n,i},
		\]
		where the \(\xi_{n,i}\)'s are independent copies of \(\xi\). Hence
		\[
	...
	q_n:=\mathbb{P}(X_n=0).
	\]
	Since \(X_n=0\) implies \(X_{n+1}=0\), the events \(\{X_n=0\}\) are increasing in \(n\). Therefore,
	\[
	q_n\uparrow q,
	\]
	where
	...
	q
	:=
	\mathbb{P}(\text{eventual extinction})
	=
	...
	Since
	\[
	q_n=\mathbb{P}(X_n=0)=G_n(0),
	\]
	we may study extinction using the iterates of the p.g.f.
    ...
    \textcolor{red}{Previously, we've appllied Stirling approximation to prove that 1D symmetric random walk is recurrent, 2D symmetric random walk is recurrent, and higher dimensional symmetric random walk is transient. Now we investigate whether the random walks are null-recurrent (the expectation of waiting time is infinite) or positive-recurrent (the expectation of waiting time is finite).}
    ...
    (\textcolor{red}{be careful that the two generating functions are not probability generating functions. For $P_{00}(s)$, its events are not disjoint. For $F_{00}(s)$, its events are defective.})
	\[
	P_{00}(s)
	:=
	...
	\begin{remark}
		...
		f_{00}^{(n)}
		\]
		is the probability of returning to \(0\) for the first time at time \(n\). Usually, \(p_{00}^{(n)}\) is easier to compute than \(f_{00}^{(n)}\) (due to Chapman-Komogorov equation).
        \textcolor{red}{We have two goals:}
        \begin{enumerate}
            \item [(1)]
            \textcolor{red}{When is $F_{00}(s)$ a probability generating function?}
            \item [(2)]
            \textcolor{red}{When $F_{00}(s)$ is a probability generating function, is the state $0$ null-recurrent or positive-recurrent?}
        \end{enumerate}
	\end{remark}
	...
	\begin{proof}
		...
		p_{00}^{(0)}=1.
		\]
		Multiplying by \(s^n\) and summing over \(n\ge 0\), we get
		\[
		P_{00}(s)
		=
		1+F_{00}(s)P_{00}(s).
	...
	\begin{proof}
		...
		F_{00}(1)=1.
		\]
		Thus the walk returns to \(0\) with probability \(1\), so state \(0\) is recurrent.
		...
		F_{00}(1)<1.
		\]
		Thus the probability of ever returning to \(0\) is strictly less than \(1\), so state \(0\) is transient.
	\end{proof}
