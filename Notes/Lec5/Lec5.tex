...
\]
Since \(\sum_{k=1}^{s} n_k=\sum_{k=1}^{s} n_k a_k=0\),
both exponential factors equal \(1\),
hence \(\frac{f(z+\omega)}{f(z)}=1\),
...
the Mittag–Leffler (ML) problem and the Weierstrass (WP) problem.
